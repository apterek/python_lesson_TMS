...
def pull_random_card(cards_sequences, suit_sequences):
    random_card = {cards_sequences[random.randint(0, len(cards_sequences) - 1)]:
                       suit_sequences[random.randint(0, len(suit_sequences) - 1)]}
    return random_card
...
def black_jack():
    ...
    while True:
        ...
        # pull more card
        game_continue = input('Enter | yes |  '
                              'if you want to take another card: ')
        # check game continue
